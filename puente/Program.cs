// mirador-puente: servidor WebSocket en 127.0.0.1 para el mundo web (ADR 0002).
//
...
string ficheroToken = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "mirador", "puente-token");
...
File.WriteAllText(config,
    "// Lo escribe mirador-puente al arrancar (ADR 0002); no se versiona.\n" +
    $"window.MIRADOR_PUENTE = {{ puerto: {PUERTO}, token: \"{token}\", atajo: {JsonSerializer.Serialize(atajo)} }};\n");
// Al arrancar (= al entrar en el mundo) los grafos se ponen al día solos, en segundo plano.
regenerador.Pedir("al entrar");
regenerador.Vigilar();
agentes.Empezar();
Console.WriteLine($"mirador-puente en ws://127.0.0.1:{PUERTO} · atajo {atajo ?? "ninguno"} · config en {config}");
await app.RunAsync();
